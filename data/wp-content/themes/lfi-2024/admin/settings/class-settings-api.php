    ...
    abstract class Settings_API
    {
        ...
        public function get_values_from_db()
        {
            if (empty($this->dbValues)) {
                ...
                $options = $this->get_fields_by_save_method('option');
                ...
                foreach ($custom as $slug => $field_options) {
                     ...
                     * @param Settings_API $menu Current menu class
        ...
        {
            foreach ($this->get_all_fields() as $key => $field_options) {
                if ($field_options['name'] != $field_name) continue;
         ...
         * @package {file_name}
         * 
         * @param string $tab 'all' or select tab
         * @return array
         **/
        private function get_all_fields($tab = 'all')
        {
            $allFields = array();
            foreach ($this->fields as $field_tab => $fields) {
                // Field not in the tab
                if ($tab != 'all' && $tab != $field_tab) continue;
                // Field in the tab
                foreach ($fields as $slug => $options) {
                    // Handle group field
                    if ($options['type'] === 'group') {
                        foreach ($options['fields'] as $key => $grpField) {
                            $allFields[] = $grpField;
                        }
                    } else {
                        // Handle the rest
         ...
         * @return array
         **/
        private function get_fields_by_save_method($method = 'option', $tab = 'all')
        {
            $allFields = $this->get_all_fields($tab);
            $options = array();
            $theme_mod = array();
            $custom = array();
        ...
        public function save_settings()
        {
            ...
            $this->posted_data = $this->_posted_data();
            $tab = isset($this->posted_data['tab']) ? $this->posted_data['tab'] : 'general';

            $option = $this->get_fields_by_save_method('option', $tab);
            foreach ($option as $slug => $field_options) {
                $dbName = $this->database_name($field_options);
                error_log(print_r($dbName, true));
            ...
            foreach ($custom as $slug => $field_options) {
                 ...
                 * @param Settings_API $menu Current menu object
                 * 
                 */
        ...
        /**
         * Posted_Data
         * Get all posted data of current tab
         *
         * @return array
         **/
        private function _posted_data()
        {
            $posted_data = $_POST;
            $tab = isset($posted_data['tab']) ? $posted_data['tab'] : 'general';
            foreach ($this->fields[$tab] as $field_name => $field_options) {
                // Checkbox
                if ($field_options['type'] == 'checkbox' && !isset($posted_data[$field_name])) {
                    $posted_data[$field_name] = 0;
                }
            }
            return $posted_data;
        }
        ...
        private function database_name($field_options = array())
        {
            ...
            if (isset($field_options['option_name'])) {
                return $field_options['option_name'];
            }
        ...
        public function validate_color($key)
        {
            ...
            // Color without input text (color name)
            // So just hexcolor to retreive
        ...
        {
            $value = $this->get_option($field['name']) !== false
